...
(function ($) {
	...
	// (end)
	//
	// Event: gridfilterdata
	// Anytime the data is being filtered, which happens with all calls to .data(newData), this event will be triggered to allow for the application
	// to filter the data.
	//
	// Parameters:
	//  e - The jQuery event object.
	...
	$.widget("surge.slickGrid", {
		...
		_create: function () {
			...
			this.element.bind("gridtextsearch.platform", function (e, searchText) {
				self.filter("gridtextsearch", function (data) {
		...
		filterData: function (data) {
			var dataResults = (data || this.options.data || []).slice(0); // we'll want to work on a clone of the data
			var self = this;
			$.each(this.filters, function (id, func) {
				if ($.isFunction(func))
					dataResults = func.call(self, dataResults, id);
			});

			this.element.trigger("gridfilterdata", dataResults);
			return dataResults;
		},
